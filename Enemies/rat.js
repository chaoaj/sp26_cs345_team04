class Rat {
  constructor(mapName) {
    ...
    // HP
    this.maxHp = 40;        // low HP
    this.hp = this.maxHp;

    // animation
    this.frame = 0;
    this.frameSpeed = 0.18; // snappy animation to match fast movement
    this.frameTimer = 0;
    this.maxFrames = 6;
  ...
  draw(sx, sy) {
    ...
    noStroke();
    fill(80, 0, 0);
    rect(x - barW / 2, y - this.size * sy / 2 - 8, barW, barH);
    fill(200, 180, 0); // yellow bar to distinguish from goblin/mage
    rect(x - barW / 2, y - this.size * sy / 2 - 8, barW * hpPercent, barH);
  }
}
